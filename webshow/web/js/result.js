...
class Result{
    ...
    loadresult(currentpage){
        ...
        let imgpath = imgpaths==""?imgurl:imgpaths[0].substr(1,imgpaths[0].length-2).split("/static/")[1];
        img.setAttribute("src","/static/"+imgpath);
    ...
    play(){
        ...
        let videopath = this.clip[1].split("/static/")[1];
        $("#myModalLabel").html(this.clip[5]);
        $("input[name='sequence']").val(this.clip[0]);
        $(".clipvideo").attr("src","/static/"+videopath);
